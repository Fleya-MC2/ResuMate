...
struct TagForm: View {
            ...
            VStack(alignment: .leading){
                ...
                if isSuggestion && !isLoading {
                    ...
                    GeometryReader { geometryProxy in
                        FlexibleView(
                            availableWidth: geometryProxy.size.width, data: skillList,
                            spacing: 15,
                            alignment: .leading
                        ) { item in
                            HStack{
    ...
    private func addTag() {
        ...
        skillList.append(newTag)
        newTag = ""
    }
    
    func removeTag(tag: String) {
        if let index = skillList.firstIndex(of: tag) {
...
struct InputTag_Previews: PreviewProvider {
    static var previews: some View {
        TagForm(skillList: .constant(["test"]))
    }
}
